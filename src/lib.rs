...
impl Plugin for SignalGeneratorPlugin {
    ...
    fn run(&mut self, n_samples: u32) {
        ...
        // Convert relf_frequency to frequency
...
impl SignalGenerator {
    fn new(rate: f32, frequency: f32) -> SignalGenerator {
        SignalGenerator {
            rate,
    ...
    fn set_frequency(self: &mut Self, frequency: f32) {
        self.new_frequency = frequency;
    }

    fn generate(self: &mut Self, output: &mut [f32]) {
        ...
        if self.new_frequency == self.current_frequency {
            ...
            println!("start frequency change");
            ...
            println!("period_t: {}, n (samples left to render with current_frequency): {}", period_t, n);
